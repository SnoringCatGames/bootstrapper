...
- Port old C++ logging to use logger.
  - Have ENSURE, PRINT, WARN, etc. write to logger when possible.

- Rename a bunch of things to "Service" (rather than controller, or handler, or no suffix (like ScaffolderAudio)).

- Ask AI to review some files.
- Ask AI to add missing test files.
- Try asking AI to add doc comments to function declarations in header files.

- Fix icon references:
  - Create bespoke icons for each framework.
  - Move from demo/ to demo/assets/icons/.
  - In .gdextension files
  - In .godot files.

- Add any missing tests.
  - Iterate through all test files and look for TODOs and any empty tests.
  - Look through all autogenerated tests and review their quality.
